...
export default function SenseiPage() {
  ...
  return (
    <div className="relative min-h-[calc(100vh-100px)] w-full overflow-hidden rounded-3xl border border-white/10 bg-zinc-950 p-6 text-zinc-100 shadow-2xl">
      {/* Mesh Space Background */}
      <div className="absolute inset-0 -z-10 bg-[radial-gradient(ellipse_at_center,rgba(16,16,28,1)_0%,rgba(5,5,10,1)_100%)]" />
      <div className="absolute inset-0 -z-10 bg-[linear-gradient(to_right,rgba(255,255,255,0.02)_1px,transparent_1px),linear-gradient(to_bottom,rgba(255,255,255,0.02)_1px,transparent_1px)] bg-[size:4rem_4rem]" />

      {/* Main Grid Layout */}
      <div className="grid h-full grid-cols-1 gap-6 lg:grid-cols-12">
        
        {/* Left Side: Siri Glowing Core & Live Transcript (Main Arena) */}
        <div className="lg:col-span-8 flex flex-col justify-between min-h-[550px] p-6 rounded-2xl bg-zinc-900/20 border border-white/5 backdrop-blur-md">
          {/* Top Status */}
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <Bot className="h-5 w-5 text-indigo-400" />
              <span className="text-xs font-semibold uppercase tracking-wider text-zinc-400">Sensei AI Tutor Core</span>
            </div>
            <div className={cn(
              "flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-medium border",
              status === "listening" ? "bg-emerald-950/40 border-emerald-500/20 text-emerald-400" :
              status === "thinking" ? "bg-indigo-950/40 border-indigo-500/20 text-indigo-400" :
              status === "speaking" ? "bg-amber-950/40 border-amber-500/20 text-amber-400" :
          ...
          <div className="flex flex-1 flex-col items-center justify-center py-12">
            <div className="relative flex h-72 w-72 items-center justify-center">
              ...
              <div className={cn(
                "absolute inset-0 rounded-full blur-3xl opacity-35 transition-all duration-1000 scale-110",
              ...
              {/* Outer Orbit Rings */}
              <div className="absolute h-64 w-64 rounded-full border border-white/5 opacity-50 scale-105" />
              <div className={cn(
                "absolute h-60 w-60 rounded-full border border-dashed border-white/10 transition-transform duration-10000",
                status === "thinking" && "animate-spin"
              )} />

              {/* The Realistic 3D Glowing Glass Orb */}
              <div className={cn(
                "relative z-10 flex h-48 w-48 items-center justify-center rounded-full shadow-2xl transition-all duration-500 border border-white/10",
                "bg-zinc-950/40 backdrop-blur-md overflow-hidden",
                status === "listening" && "shadow-emerald-500/20 border-emerald-500/20 scale-105",
                status === "thinking" && "shadow-indigo-500/30 border-indigo-500/20 scale-110",
                status === "speaking" && "shadow-amber-500/20 border-amber-500/20 scale-105"
              )}>
                
                {/* 3D Glass Gloss Overlay (Deepens the 3D shape) */}
                <div className="absolute inset-0 z-30 pointer-events-none rounded-full" 
                     style={{
                       background: "radial-gradient(circle at 35% 35%, rgba(255, 255, 255, 0.15) 0%, rgba(255, 255, 255, 0) 50%), radial-gradient(circle at 50% 80%, rgba(255, 255, 255, 0.05) 0%, rgba(255, 255, 255, 0) 40%)"
                     }} />
                ...
                <div className={cn(
                  "orb-layer-2 absolute inset-4 rounded-full blur-lg mix-blend-screen opacity-80 transition-colors duration-1000",
                ...
                <div className="relative z-40 transition-all duration-500 scale-100 opacity-60">
                  <div className={cn(
                ...
                {[1, 2, 3, 4, 5, 6, 7, 8, 9].map((bar) => (
                  <div
                    ...
                    className={cn(
                      ...
                      status === "thinking" ? "bg-indigo-400 animate-pulse" :
                      status === "speaking" ? "bg-amber-400" :
                      "bg-zinc-700",
          ...
          <div className="w-full border-t border-white/5 pt-4 text-center">
            <p className={cn(
              ...
              status === "speaking" ? "text-amber-200" :
              status === "thinking" ? "text-indigo-300 italic" :
              status === "listening" ? "text-emerald-100" : "text-zinc-500"
            )}>
              {activeSpeech}
            </p>
          </div>
        ...
        <div className="lg:col-span-4 flex flex-col justify-between gap-6">
          ...
          {/* Active Context Card */}
          <div className="p-5 rounded-2xl bg-zinc-900/30 border border-white/5 backdrop-blur-md">
            ...
            <div className="space-y-3 text-xs">
              <div className="flex justify-between border-b border-white/5 pb-2">
                ...
                <span className="font-semibold text-zinc-200">MSc Sound & Music AI</span>
              </div>
              <div className="flex justify-between border-b border-white/5 pb-2">
                <span className="text-zinc-500">Target intake</span>
                <span className="font-semibold text-orange-400">Barcelona, Spain</span>
              </div>
              <div className="flex justify-between border-b border-white/5 pb-2">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-semibold text-indigo-400">IELTS Prep & Audio Projects</span>
              </div>
            </div>
          </div>

          {/* Quick Interactive Spanish Trigger Cards */}
          <div className="p-5 rounded-2xl bg-zinc-900/30 border border-white/5 backdrop-blur-md flex-1 flex flex-col justify-between">
            <div>
              <h4 className="text-xs font-bold uppercase tracking-wider text-zinc-400 mb-3 flex items-center gap-2">
              ...
              <div className="space-y-2">
                <button
                  ...
                  className="w-full flex items-center justify-between rounded-xl bg-white/5 border border-white/5 px-3 py-2 text-left text-xs text-zinc-300 hover:bg-white/10 hover:border-white/10 transition-all group"
                >
            ...
            <div className="mt-4 border-t border-white/5 pt-4">
              ...
              <div className="h-36 overflow-y-auto rounded-lg bg-black/60 p-2 font-mono text-[9px] text-zinc-400 space-y-1 scrollbar-thin">
                {events.length === 0 ? (
                  <div className="text-zinc-600 text-center py-8">Waiting for audio daemon...</div>
                ) : (
